...
import type { GlassesScreen, PaceRailSettings, Snapshot, TransportKind } from './types'
 ...
 */
function statusLine(view: WorkoutView, kind: TransportKind, settings: PaceRailSettings): string {
 ...
 */
export function runScreen(view: WorkoutView, kind: TransportKind, settings: PaceRailSettings): ScreenSpec {
  const snap = view.snapshot
  const u = settings.units
  const paceUnit = paceUnitLabel(u)

  const text: TextBox[] = [
    tile(1, 'pr-dist', 0, 0, u === 'mi' ? 'DIST mi' : 'DIST km',
      fmtDistance(snap?.distance ?? null, u), L.primary),
    tile(2, 'pr-time', 1, 0, 'TIME',
      fmtDuration(snap?.elapsed ?? null), L.primary),
    tile(3, 'pr-hr', 2, 0, 'HR',
      zoneNow(view), L.primary),

    tile(4, 'pr-pace', 0, 1, `PACE ${paceUnit}`,
      fmtPace(snap?.paceSecPerKm ?? null, u), L.primary),
    tile(5, 'pr-avg', 1, 1, `AVG ${paceUnit}`,
      fmtPace(view.avgPaceSecPerKm, u), L.secondary),
    tile(6, 'pr-cal', 2, 1, 'CAL',
      snap?.energy != null ? String(Math.round(snap.energy)) : '—', L.secondary),

    wide(7, 'pr-step', STEP_Y, STEP_H, stepBlock(view, settings), L.primary),
    wide(8, 'pr-foot', FOOT_Y, TILE_H,
      fitLines([
 ...
 */
function stepBlock(view: WorkoutView, settings: PaceRailSettings): string {
 ...
 */
export function splitsScreen(view: WorkoutView, kind: TransportKind, settings: PaceRailSettings): ScreenSpec {
  const u = settings.units
  ...
  return {
    screen: 'splits',
    text: [
      wide(1, 'pr-splits-h', 2, LINE, fit(`Splits — ${view.splits.length} · ${settings.planName}`, SCREEN_W - 12), L.secondary),
      wide(8, 'pr-splits-f', 252, LINE, fit(statusLine(view, kind, settings), SCREEN_W - 12), L.chrome),
    ],
    lists: [{
      id: 2,
      name: 'pr-splits-l',
 ...
 */
export function zonesScreen(view: WorkoutView, kind: TransportKind, settings: PaceRailSettings): ScreenSpec {
  const z = view.zones
  const header = z
  ...
  return {
    screen: 'zones',
    text: [
      wide(1, 'pr-zones-h', 2, LINE, fit(header, SCREEN_W - 12), L.secondary),
      wide(8, 'pr-zones-f', 252, LINE, fit(statusLine(view, kind, settings), SCREEN_W - 12), L.chrome),
    ],
    lists: [{ id: 2, name: 'pr-zones-l', x: 6, y: 34, w: SCREEN_W - 12, h: 212, items, capture: true }],
    images: [],
  }
}
...
export function mapScreen(
  ...
  const text: TextBox[] = [
    ...
    wide(7, 'pr-map-a', 140, LINE,
      ...
      L.primary),
    wide(8, 'pr-map-f', 200, 2 * LINE, mapFootLines(view, kind, settings, map), L.chrome, true),
  ]

  const images: ImageBox[] = hasMap
    ? [{ id: 3, name: 'pr-map-img', x: Math.round((SCREEN_W - MAP_W) / 2), y: 34, w: MAP_W, h: MAP_H }]
    : []
